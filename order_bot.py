...
async def handle_order(request: web.Request):
    ...
    text += f"Тип клиента: {customer_type}\n"
    text += f"Получение: {delivery}"
    if delivery == "Доставка" and delivery_method:
        text += f" — {delivery_method}"
    text += "\n"
    if delivery == "Доставка" and address:
